...
class UtilsModule(private val context: Context) {
    ...
    @Singleton
    @Provides
    fun provideLatestNewsDb(): EhHoDatabase = Room.databaseBuilder(
        context, EhHoDatabase::class.java, "ehHoDatabase-database"
    ).build()
...
fun provideOkHttpClient(sp: SharedPreferences): OkHttpClient {
    val interceptor = object : Interceptor {
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            println("lolololo"+ sp.getString(PREFERENCES_SESSION_USERNAME, ""))
